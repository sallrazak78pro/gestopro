...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const denied = requirePermission(ctx, "boutiques", "view");
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { ctx, error } = await getTenantContext();
    if (error) return error;
    const denied = requirePermission(ctx, "boutiques", "edit");
    if (denied) return denied;
    await connectDB();
    ...
    await logActivity({
      ...
      action: ACTIONS.BOUTIQUE_MODIFIEE, module: MODULES.BOUTIQUES,
      details: `Boutique modifiée — ${boutique.nom}`,
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { ctx, error } = await getTenantContext();
    if (error) return error;
    const denied = requirePermission(ctx, "boutiques", "delete");
    if (denied) return denied;
    await connectDB();
